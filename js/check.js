      ...
      sh02: {
        ...
        analytics: [
          ...
          },
          {
            ...
            hits: 0,
            findings: [],
            interp: '<b>Not applicable for T1003.001.</b> LSASS credential dumping via reflective DLL injection and NtDuplicateObject handle theft does not produce meaningful command line artifacts — the tool is loaded into memory without a visible command string. Process Chain Anomaly (above) and the Sysmon EventCode=10 query are the correct detection paths for this technique.',
          },
        ],
      },
      sh03: {
...
function renderRAAResults(dataOverride) {
  ...
  if (!d.relevant) {
    bodyHTML = `<div class="raa-no-coverage">
      ...
      <span><b style="color:var(--text);">No retrieved alert coverage for this hypothesis</b><br>${d.reason}</span>
    </div>`;
  } else {
  ...
  const subhuntBanner = subhuntLabel
    ? `<div style="display:flex;align-items:center;gap:8px;padding:7px 15px;background:rgba(59,130,246,.05);border-bottom:1px solid rgba(59,130,246,.12);font-size:11px;color:var(--muted);">
        <span>Investigation scope for</span>
