...
const keepData = {
  ...
  '040': {
    ...
    pivot: {
      ...
      seedData: ['WIN-WS012 NTLM auth log (48h post-infection)', 'Finance segment host list from CMDB', 'LSASS dump artifact hashes (T1003.001 finding)']
    }
  },
  '039': {
    ...
    subhuntLock: {
      sh01: {
        ...
        o: 'Unsigned binary introduced on SRV-BUILD01 Apr 10. SHA-256 mismatch against artifact registry. 4-day exposure window before detection. 10 downstream hosts received the payload.',
    ...
    report: {
      ...
      approach: 'H-01 (trojanised build artifact) was confirmed by cross-referencing Sysmon file-creation events on SRV-BUILD01 with known-good binary hashes from the artifact registry — an unsigned binary introduced on Apr 10 did not match any registered build output. H-02 (C2 staging) was confirmed via JA3 fingerprint on outbound HTTPS from SRV-BUILD01 to <span class="report-ioc">update.cdn-cache[.]net</span>, matching a Cobalt Strike malleable profile. DLL sideloading via a legitimate Windows binary was identified on 10 downstream hosts that had pulled the affected build.',
      impact: [
    ...
    pivot: {
      ...
      seedData: ['Affected downstream host list (10 hosts)', 'Apr 10–14 artifact hash inventory from artifact registry', 'Sysmon DLL load events (index=sysmon EventCode=7) since Apr 10']
    }
  }
};
