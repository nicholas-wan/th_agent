...
// Snapshots of Stage 0–4 original content — captured at first load
...
function updateAgentPills(i) {
  const pills = [
    ...
    { hyp:['Running','chip-blue',30,'Analysing CTI · 8 TTPs queued'],               orch:['Running','chip-blue',50,'Routing tasks · pipeline initialised'],  ts:['Waiting','chip-gray',0,'Awaiting TTPs'],               dl:['Waiting','chip-gray',0,'Awaiting investigation notes'] },
    // step 1: TTPs loaded from upstream pipeline, hypothesis agent working
    { hyp:['Running','chip-blue',70,'Cross-referencing past hunts · 3 recalled'],     orch:['Running','chip-blue',70,'Routing tasks · 2 agents active'],        ts:['Waiting','chip-gray',0,'Awaiting hypotheses'],          dl:['Waiting','chip-gray',0,'Awaiting investigation notes'] },
    // step 2: hypotheses generated by Hypothesis Agent
    { hyp:['Done','chip-green',100,'4 hypotheses generated · handed to Orchestrator'],orch:['Done','chip-green',100,'Hypotheses routed · pipeline continues'],   ts:['Review','chip-blue',80,'4 hypotheses ready for review'], dl:['Waiting','chip-gray',0,'Awaiting investigation notes'] },
    // step 3: investigation running
    { hyp:['Done','chip-green',100,'4 hypotheses generated · analyst notes applied'], orch:['Done','chip-green',100,'Hunt coordinated · all tasks routed'],     ts:['Active','chip-blue',100,'Retrieving alerts · 59 hits'],  dl:['Running','chip-blue',40,'Awaiting investigation notes'] },
    // step 4: detection logic + rule validation (via tool)
    { hyp:['Done','chip-green',100,'4 hypotheses generated · analyst notes applied'], orch:['Done','chip-green',100,'Hunt coordinated · all tasks routed'],     ts:['Done','chip-green',100,'Investigation complete'],  dl:['Active','chip-blue',90,'Generating rules · validating via RuleVal tool…'] },
  ];
...
const feedAgentColors = {
  ...
  data: { bg:'rgba(249,115,22,.18)', border:'#f97316' },
...
const feedStepLabels = ['Select Intel','TTP Extraction','Hypotheses','Investigation','Detection Logic'];
let feedTimers = [];
...
function clearAgentsFeed() {
  ...
  _totalTokens = 0;
  // Reset legend
  ['orch','hyp','ts','dl'].forEach(a => _setAgentLegendStatus(a, 'idle'));
...
const _topoLineMap = {
  'hyp':'topo-l-hyp','ts':'topo-l-ts','dl':'topo-l-dl'
};
...
const closedHuntFeeds = {
  '040': [
    ...
    { type:'relay',  agent:'orch', to:'ts',  msg:'2 hypotheses analyst-approved · T1566.001 (delivery) + T1490 (staging) · begin investigation' },
    { sep: 'Investigation' },
  ...
  '039': [
    ...
    { type:'relay',  agent:'orch', to:'ts',  msg:'2 hypotheses analyst-approved · T1195.002 (root cause) + T1071.001 (active C2) · begin investigation' },
    { sep: 'Investigation' },
...
const closedLearnData = {
  '040': {
    ...
    rules: [
      { id:'DL-2026-040-001', ttp:'T1566.001', name:'Office macro exec', sev:'HIGH' },
      { id:'DL-2026-040-002', ttp:'T1490',     name:'vssadmin shadow deletion', sev:'HIGH' },
      { id:'DL-2026-040-003', ttp:'T1204',     name:'User execution macro', sev:'HIGH' },
      { id:'DL-2026-040-004', ttp:'T1574.002', name:'DISM sideload', sev:'HIGH' },
      { id:'DL-2026-040-005', ttp:'T1486',     name:'File encryption pattern', sev:'HIGH' },
    ],
  },
  '042': {
  ...
  '039': {
    ...
    rules: [
      { id:'DL-2026-039-001', ttp:'T1195.002', name:'Unsigned build artifact', sev:'HIGH' },
      { id:'DL-2026-039-002', ttp:'T1574.002', name:'DLL sideloading', sev:'HIGH' },
      { id:'DL-2026-039-003', ttp:'T1071.001', name:'CS C2 JA3 beacon', sev:'HIGH' },
      { id:'DL-2026-039-004', ttp:'T1053.005', name:'Scheduled task persistence', sev:'HIGH' },
    ],
  },
};
